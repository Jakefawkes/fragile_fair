...
def get_fogliato_metrics_autobounds(
    ...
    variable_list = [outcome_node, prediction_node, attribute_node]
    if metric == "FPR":
...
def Fogliato_true_bounds(
    ...
    if not standard:
        ...
        prob_vec = get_p_vector_torch(reversed_df,
        prediction_node=prediction_node, attribute_node="A", outcome_node=outcome_node)

    else:
        prob_vec = get_p_vector_torch(observed_joint_table,
            prediction_node=prediction_node, attribute_node="A", outcome_node=outcome_node)

    return get_fogliato_true_bounds(prob_vec,sensitivity_parameter_value = sensitivity_parameter_value,
